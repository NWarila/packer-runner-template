...
|---|---|---|
| `.github/workflows/pr-verify.yaml` | `pull_request` | Calls `nwarila-platform/packer-framework-template/.github/workflows/reusable-packer-framework-build.yaml@<sha>` in `mode: validate`. The framework checks out itself, overlays the runner's inputs, runs `packer init` + `validate-safe` + `inspect`. |
